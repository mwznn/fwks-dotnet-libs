...
// TODO: Add Logger toggle for putting info when actions fail.
...
public sealed class RedisService : IRedisService
{
    ...
    {
        _connection.Subscriber.Subscribe(channel, (_, value) => action(channel, value), commandFlag.ToEnum<CommandFlags>());
    }

    public Task SubscribeAsync(string channel, Action<string, string> action, string commandFlag = "None")
    {
        return _connection.Subscriber.SubscribeAsync(channel, (_, value) => action(channel, value), commandFlag.ToEnum<CommandFlags>());
    }

    public void Publish(string channel, object content, string commandFlag = "None")
    {
        _connection.Subscriber.Publish(channel, content.SerializeObject(), commandFlag.ToEnum<CommandFlags>());
    }

    public Task PublishAsync(string channel, object content, string commandFlag = "None")
    {
        return _connection.Subscriber.PublishAsync(channel, content.SerializeObject(), commandFlag.ToEnum<CommandFlags>());
    }
}
